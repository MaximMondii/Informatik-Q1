...
public class Queue<ContentType> {
    ...
    public void enqueue(ContentType pContent) {
        //Einen Knoten anlegen, wenn etwas in p
        if(pContent != null){
            QueueNode node = new QueueNode(pContent);
            //Den alten Tail auf den neuen Knoten zeigen lassen
            if (tail!= null){
                tail.setNext(node);
                //Den Knoten hinten einfügen
                tail = node;
            }

            //Falls dies das erste Element ist, muss es auch der head sein
            if (isEmpty()){
                head = node;
                tail = node;
            }
     ...
     * Falls es sich um das einzige Element in der Schlange handelt (head == tail), werden beide Verweise auf 
     * null gesetzt.
     */
    public void dequeue() {
        if(!isEmpty()){
